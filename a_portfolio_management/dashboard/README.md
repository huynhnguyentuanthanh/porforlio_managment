...
A Dockerized FastAPI dashboard for:

- reading weekly CSV outputs from `market_collector`
- merging datasets on weekly `date`
- computing Gold / SJC / VN30 indicators
- rendering charts and data tables in a single-page UI

This README documents the **dashboard architecture** and the **full contents of all key files** in `dashboard/`.

---

# Part 1 — Dashboard Responsibilities
...
```bash
/data/market_collector/outputs/
```

Inside Docker, this comes from the volume mount:

```yaml
- ..:/data
```

So the dashboard expects the collector outputs to exist at:
...
# Part 2 — Expected Input Series

The dashboard loads these single-series files:
...
All input data is assumed to already be:
...
- outer-merges all datasets on `date`

## Date filtering

The merged data is filtered by:
...
For Gold / SJC / VN30, the dashboard computes:
...
Default visible series on the main chart:

- `vn30_scaled`
- `gold_scaled`
- `us3m`

Other series are loaded but hidden by default.

---

# Part 4 — File Contents
...
- default visible tickers remain:
  - `vn30_scaled`
  - `gold_scaled`
  - `us3m`
- other series such as `dxy`, `vix`, `wti` and `sp500` are loaded into the merged table and main chart, but hidden by default
